...
class GKTodayScraper:
    ...
    def scrape_articles(self):
        ...
        for heading in headings:
            ...
            if not title or len(title) < 10 or title in seen_titles: continue
    ...
    def _parse_full_article_page(self, html, title, url):
        ...
        for p in paragraphs:
            if p.find_parent(class_=['sharedaddy', 'related-posts']): continue
    ...
    def generate(self, data, path=None):
        ...
        for sec in data.get('sections', []):
            ...
            for art in sec['articles']:
                if art['relevance']['level'] == 'HIGH': story.append(Paragraph("HIGH YIELD", self.styles['GKBadgeH']))
...
class Pipeline:
    ...
    def _telegram(self, data, pdf_path):
        ...
        try:
            msg = f"📚 <b>GKToday Deep Digest: {data['display_date']}</b>\n\n📝 Extracted {data['total_articles']} full articles from the last 48 hours."
            req.post(f"https://api.telegram.org/bot{self.tg_token}/sendMessage", json={'chat_id': self.tg_chat, 'text': msg, 'parse_mode': 'HTML'}, timeout=30)
            with open(pdf_path, 'rb') as f:
                req.post(f"https://api.telegram.org/bot{self.tg_token}/sendDocument", files={'document': f}, data={'chat_id': self.tg_chat}, timeout=60)
